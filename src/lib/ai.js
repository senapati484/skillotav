...
async function fetchGitHubData(username) {
  try {
    ...
    if (!reposRes.ok)
      throw new Error(`Repositories error: ${reposRes.statusText}`);
...
function generateRolePrompt(githubData) {
  const totalBytes = Object.values(githubData.languages).reduce(
    (a, b) => a + b,
    0
  );
  ...
  const repoTypes = githubData.repos.reduce(
    (acc, repo) => {
      const isFrontend = repo.topics?.some((t) => t.match(/frontend|ui|ux/i));
      const isBackend = repo.topics?.some((t) =>
        t.match(/backend|api|server/i)
      );
...
${langPercentages
  .slice(0, 5)
  .map((l) => `   - ${l.lang.padEnd(15)} ${l.percent}%`)
  .join("\n")}
...
3. **Activity Analysis**:
   ...
   - Documentation score: ${Math.min(
     Math.round(githubData.repos.length * 0.2),
     5
   )}/5
...
function validateGeminiResponse(text) {
  try {
    ...
    const pattern =
      /Best Role: (.+) \| Confidence: (High|Medium|Low) \| Recommended Skills: (.+)/i;
    ...
    if (
      !Array.from(VALID_ROLES).some((r) => r.toLowerCase() === normalizedRole)
    ) {
...
function validateResumeResponse(text) {
  const pattern =
    /Best Role: (.+) \| Confidence: (High|Medium|Low) \| Key Factors: (.+)/i;
...
export function calculateFitScore(githubData, resumeData) {
  ...
  const roleMatchBonus =
    githubData.role === resumeData.role
      ? 0.15
  ...
  const totalScore = Math.min(
    Math.max(githubScore + resumeScore + roleMatchBonus + skillBonus, 0),
    1
  );
...
function rolesAreRelated(role1, role2) {
  ...
  return (
    roleGroups[role1]?.includes(role2) || roleGroups[role2]?.includes(role1)
  );
}
...
function calculateSkillOverlap(githubSkills, resumeFactors) {
  ...
  return (
    normalizedSkills.filter((s) => normalizedFactors.some((f) => f.includes(s)))
...
export function determineFinalRole(githubAnalysis, resumeAnalysis) {
  ...
  return githubScore >= resumeScore ? githubAnalysis.role : resumeAnalysis.role;
}
